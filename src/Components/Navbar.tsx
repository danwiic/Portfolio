...
export default function Navbar({ children }: { children: React.ReactNode }) {
  return (
    <>
      <div className="flex justify-between bg-white p-6 py-7 shadow-md text-md mb-1 sticky top-0 z-10">
        <div>Danwiic</div>
        ...
        return (
          <li key={i}>
            <a
              ...
              className={`hover:text-pink-800 before:rounded-xl px-4 py-7 relative before:absolute before:left-0 before:bottom-0 before:h-1 before:w-0 before:bg-pink-800 hover:before:w-full before:transition-all before:duration-500 ease-in-out ${
                activeLink === item.toLowerCase() ? 'text-pink-800 before:w-full' : ''
              }`}
              onClick={(e) => {
